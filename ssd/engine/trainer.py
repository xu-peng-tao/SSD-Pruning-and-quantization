...
import prune.normal_prune as normal_prune
from ssd.engine.inference import do_evaluation
from ssd.utils import dist_util
from ssd.utils.metric_logger import MetricLogger
import prune.bn_sparse as bn_sparse
import prune.sortcut_prune as shortcut_prune
...
def do_train(cfg, model,
    ...
    if cfg.PRUNE.TYPE != 'no':
        ...
        if cfg.PRUNE.TYPE == 'normal':
            ...
            _, _, prune_idx = normal_prune.parse_module_defs(backbone.module_defs)
        elif cfg.PRUNE.TYPE == 'shortcut':
            logger.info('shortcut sparse training')
            _, _, prune_idx, _, _= shortcut_prune.parse_module_defs(backbone.module_defs)
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        if cfg.PRUNE.TYPE != 'no':
            if hasattr(model, 'module'):
                bn_sparse.updateBN(model.module.backbone.module_list, cfg.PRUNE.SR, prune_idx)
            else:
                # print(model.backbone.module_list)
                bn_sparse.updateBN(model.backbone.module_list, cfg.PRUNE.SR, prune_idx)
        ...
        if args.eval_step > 0 and iteration % args.eval_step == 0 and not iteration == max_iter:
            eval_results = do_evaluation(cfg, model, distributed=False, iteration=iteration)#单gpu测试
